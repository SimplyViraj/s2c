...
const Navbar = () => {
    ...
    const tabs: TabProps[] = [
        { label: 'Canvas', href: `/dashboard//canvas?project=${projectId}`, icon: <HashIcon className="h-4 w-4" /> },
    ...
    return (
        <div className='grid grid-cols-2 lg:grid-cols-3 p-6 fixed top-0 left-0 right-0 z-50'>
            <div className='flex gap-4 items-center'>
                ...
                !hasStyleGuide && (
                    <div className="lg:inline-block hidden rounded-full text-primary/60 border border-white/[0.12] backdrop-blur-xl
                    bg-white/[0.08] px-4 py-2 text-sm saturate-150">
                        Project/{project?.name}
                    </div>
                ))}
                </div>
                <div className='lg:flex hidden items-center gap-2 justify-center'>
                    <div className='flex items-center gap-2 backdrop-blur-xl bg-white/[0.08] border border-white/[0.12] rounded-full p-2 saturate-150'>
                        {tabs.map((tab) => (
                            <Link
                                ...
                                href={tab.href}
                                className={[
                                    'group inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm transition',
                ...
                <div className='flex items-center gap-4 justify-end'>
                    ...
                    <Button 
                        variant="secondary"
                        className='rounded-full h-12 w-12 flex items-center justify-center backdrop-blur-xl bg-white/[0.08] border border-white/[0.12] saturate-150
                         hover:bg-white/[0.12]'>
